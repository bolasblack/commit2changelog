...
// 使用方式：node changelog.js 项目地址 开始commit  [结束commit]
...
var GIT_DIR = '--git-dir=' + (process.argv[2] || process.cwd()).replace(/\/?$/, '/.git');
var GIT_HOST = '';
...
var GIT_TAG_CMD = 'git ' + GIT_DIR + ' describe --tags --abbrev=0';
...
var linkToIssue = function(issue) {
  return util.format(LINK_ISSUE, issue, GIT_HOST, issue);
};


var linkToCommit = function(hash) {
  return util.format(LINK_COMMIT, hash.substr(0, 8), GIT_HOST, hash);
};
...
var readGitLog = function(grep) {
  var deferred = q.defer();

  // TODO(vojta): if it's slow, use spawn and stream it instead
  child.exec(util.format(GIT_LOG_CMD, grep, '%H%n%s%n%b%n==END=='), function(code, stdout, stderr) {
    var commits = [];
...
var generate = function(version, file) {
  ...
  readGitLog('^fix|^feat|Breaks').then(function(commits) {
    console.log('Parsed', commits.length, 'commits');
    console.log('Generating changelog to', file || 'stdout', '(', version, ')');
    writeChangelog(file ? fs.createWriteStream(file) : process.stdout, commits, version);
  });
...
child.exec(GIT_HOST_CMD, function(error, stdout, stderr) {
